...
class Interpreter(InterpreterBase):
    ...
    def obj_get_var(self, name):
        ...
        if status:
            if split_name[1] == 'proto' and val.val is None:
                super().error(
                    ErrorType.NAME_ERROR,
                    f"Variable {name} has not been defined",
                )
    ...
    # Get the object POINTER bound to the name if it exists:
    # can you capture functions? no
    def get_obj(self, name):
        if self.is_name_obj(name):
            split_name = name.split('.')
            obj = self.env.get(split_name[0])
            if type(obj) != dict:
                super().error(
                    ErrorType.TYPE_ERROR,
                    f"object {name} is not an object",
    ...
    def parse_single(self, arg):
        ...
        if arg.elem_type in self.bin_ops or arg.elem_type in self.un_ops or arg.elem_type == 'fcall' or arg.elem_type == 'mcall' or arg.elem_type == 'lambda':
    ...
    def method_search(self, obj, name):
        ...
        if 'proto' in obj and obj['proto'].val is not None:
            return self.method_search(obj['proto'].val, name)
        return None, False
        ...
        if func.elem_type == 'lambda':
            for i in range(len(args)):
                ...
                # must be a refarg
                    ...
                    if self.var_or_none(args[i]) != None:
                        # object name check needs to be added to var_or_none
                        func.get('cap_env')[arg_name] = self.get_obj(self.var_or_none(args[i]))
                    else:
                        #you're defining a lambda in the argument
                        if 'cap_env' not in func.dict:
                            func.dict['cap_env'] = self.flatten_env(self.env)
                        func.get('cap_env')[arg_name] = Box(self.parse_single(args[i]))
            ...
            #self.set_var("this", obj, True, False)
            self.env.environment[self.env.curr_scope]['this'] = self.get_obj(objref)
            val = self.execute_function(func)
            self.env.set_ret(False)
            self.env.exit_block()
            return copy.deepcopy(val)
